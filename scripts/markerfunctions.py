...
def get_vectors(image, points, mtx, dist):
    ...
    objp = np.transpose( np.expand_dims(objp, axis=2), (0, 2, 1) )
    #objp = np.transpose(objp, (1,3,2));
    imgp = np.transpose( np.expand_dims(imgp, axis=2), (0, 2, 1) )
    _, rvecs, tvecs, inliers = cv2.solvePnPRansac(objp, imgp, mtx, dist)
    #print(cv2.solvePnPRansac(objp, imgp, mtx, dist))
